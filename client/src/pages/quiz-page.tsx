...
const QuizPage = () => {
  ...
  const handlePreviousQuestion = () => {
    if (currentQuestion > 0) {
      setCurrentQuestion(prev => prev - 1);
      setSelectedOptions(
        answers[quiz!.questions[currentQuestion - 1].id] as string[] || []
      );
      setShowExplanation(false);
    }
  };
    ...
    if (!quizCompleted) {
      ...
      toast({
        ...
        description: `Your score: ${finalScore}%. ${
          finalScore >= (quiz?.passingScore || 70) 
            ? "Congratulations! You passed the quiz." 
            : "Keep practicing to improve your score."
        }`,
        variant: finalScore >= (quiz?.passingScore || 70) ? "default" : "destructive"
      });
    }
  };
            ...
            >
              <Card className="overflow-hidden">
                <div className="p-6 text-center bg-gradient-to-r from-primary/10 to-secondary/10 dark:from-primary/20 dark:to-secondary/20">
                  ...
                  <p className="text-gray-600 dark:text-gray-300 mb-6">{quiz.title}</p>
